...
const APP_SHELL=[
  ...
  './js/v2/app-state.js','./js/v2/router.js','./js/v2/role-config.js','./js/v2/storage.js','./js/v2/auth.js','./js/v2/updates.js','./js/v2/app.js',
...
self.addEventListener('activate',event=>{
  event.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('chebsel-v2-')&&!([STATIC_CACHE,HTML_CACHE].includes(k))).map(k=>caches.delete(k)));
...
async function networkFirst(request){
  ...
  catch{ return (await cache.match(request))||(await caches.match('./index.html')) }
}
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate'||url.pathname.endsWith('.html'))event.respondWith(networkFirst(event.request));
  else event.respondWith(cacheFirstRefresh(event.request));
});
