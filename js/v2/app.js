...
function showRolePicker(){const modal=$('#rolePicker');if(!modal)return;modal.hidden=false;modal.querySelectorAll('[data-role]').forEach(btn=>btn.onclick=()=>{const role=btn.dataset.role;signIn({role,user:{name:btn.dataset.name||getRoleConfig(role).label}});modal.hidden=true;resetTo('home');renderHeader()})}
...
export async function initApp(){
  ...
  try{
    initAuth();loadLocalData();initRouter();registerCoreRoutes();subscribe(renderHeader);renderHeader();void renderCurrent();if(!AppState.user)showRolePicker();setState({boot:{ready:true,error:null}});
