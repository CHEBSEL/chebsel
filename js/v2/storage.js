...
export function saveMembersCompatible(members){
  ...
  if(attendanceRaw&&typeof attendanceRaw==='object'&&!Array.isArray(attendanceRaw))save(KEYS.attendance,{...attendanceRaw,members:list.map(m=>({...m}))});
  const financeRaw=parse(KEYS.finance,null);
  if(financeRaw&&typeof financeRaw==='object'&&!Array.isArray(financeRaw))save(KEYS.finance,{...financeRaw,members:list.map(m=>({...m}))});
  return list;
}
...
export function saveAttendanceCompatible(calls,settingsPatch=null){
  ...
  if(settingsPatch&&typeof settingsPatch==='object')next.settings={...(current.settings||{}),...settingsPatch};
