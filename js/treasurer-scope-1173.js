...
(function(){
 ...
 window.closeTreasurerHub=id=>document.getElementById(id)?.classList.remove('open');

 window.openTreasurerPaymentHub=function(){
  if(role()!=='treasurer'){if(typeof openTreasuryPaymentHub==='function')return openTreasuryPaymentHub();return}
  treasurerBackTarget=null;
  const v=ensureHub('treasurerPaymentHub1173','Paiements cotisations & amendes'),m=document.getElementById('treasurerPaymentHub1173Menu');
 ...
 const obs=new MutationObserver(()=>{if(financeScope)applyFinanceScope()});document.addEventListener('DOMContentLoaded',()=>obs.observe(document.body,{subtree:true,childList:true}));
})();
